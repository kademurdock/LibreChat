...
function scrubCaption(text: string): string {
  ...
  // Content/Parts/Text.tsx and MessageContent.tsx, hideDanglingVoiceTag AND
  // hideDanglingCitationMarker both apply unconditionally here rather than
  // being gated on isSubmitting/isLatestMessage (July 19 2026, Kade: tags
  // flashing in the web chat view -- captions are the other live surface
  // with the same not-yet-closed-tag gap; see hideDanglingVoiceTag's and
  // hideDanglingCitationMarker's own doc comments). hideDanglingCitationMarker
  // also newly covers this function's pre-existing gap of never stripping
  // SPAN_REGEX/COMPOSITE_REGEX (the composite/highlighted-span citation
  // forms) at all -- it strips any complete pair the same way those two
  // regexes would, then additionally hides a still-open, not-yet-closed one.
  return hideDanglingCitationMarker(hideDanglingVoiceTag(stripGameSoundTags(stripVoiceTags(text))))
