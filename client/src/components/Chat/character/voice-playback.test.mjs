...
// Sep 19 2026: expression sheets. Whole faces dissolve; mouths take shapes while talking.
{
  ...
  const { preparedPortrait, KIANA_ID, KIANA_PORTRAIT_FILE, DELLA_ID, DELLA_PORTRAIT_FILE, LILLY_ID, LILLY_PORTRAIT_FILE } = await import('./portrait-rig.mjs');
  ...
  test('sheet panels and every registered region stay inside the artwork', () => {
    ...
    for (const [id, file] of [[KIANA_ID, KIANA_PORTRAIT_FILE], [DELLA_ID, DELLA_PORTRAIT_FILE], [LILLY_ID, LILLY_PORTRAIT_FILE]]) {
