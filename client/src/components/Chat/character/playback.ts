...
export function createPresentationRelay(current: () => CallPresentation | undefined): CallPresentation {
  return {
...
export function presentationStatus(observer: CallPresentation | undefined, status: PresentationStatus) {
  try { observer?.status(status); } catch { /* Presentation cannot interrupt a call. */ }
}

export function clearPresentation(observer: CallPresentation | undefined) {
  try { observer?.clear(); } catch { /* Presentation cannot interrupt a call. */ }
}

export function selectPresentation(observer: CallPresentation | undefined, agentId: string | null) {
  try { observer?.select(agentId); } catch { /* Keep the existing portrait. */ }
}
...
export function observePlayback(
  ...
  try {
    ...
    if (ended) source.addEventListener('ended', () => {
      try { ended(); } catch { /* Audio cleanup still runs. */ }
    }, { once: true });
  } catch { /* Malformed visual data must not break the playback queue. */ }
}
