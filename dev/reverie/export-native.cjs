...
(async()=>{
 ...
 const bundle=await rollup({input:path.join(assets,'stage.mjs'),plugins:[{
  name:'local-versioned-modules',
  resolveId(source, importer) {
   if (importer && source.startsWith('./') && /\?v=\d+$/.test(source))
    return path.resolve(path.dirname(importer), source.replace(/\?v=\d+$/, ''));
  }
 },terser()]});
