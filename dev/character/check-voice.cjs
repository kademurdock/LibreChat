...
await page.addInitScript(()=>{window.__voiceFrames=0;const original=requestAnimationFrame;window.requestAnimationFrame=callback=>original(t=>{window.__voiceFrames++;callback(t);});});
...
await page.locator('img').dispatchEvent('error');await page.waitForFunction(()=>document.querySelectorAll('canvas').length===0);await page.waitForTimeout(200);const frames=await page.evaluate(()=>window.__voiceFrames);await page.waitForTimeout(250);assert.equal(await page.evaluate(()=>window.__voiceFrames),frames);assert.equal(await page.evaluate(()=>window.voiceFixture.audio.paused),false);
fs.writeFileSync(path.join(out,'voice-browser.json'),JSON.stringify({passed:true,failedImageStopsFrames:true,paused,noNewRequestsOnResume:true,errors,requests,tree},null,2));await browser.close();server.close();console.log('Voice-message browser checks pass: playback, pause/resume without requests, reduced/off, identity, interrupt, layouts, accessibility and image-failure cleanup.');
