...
await build({configFile:false,root,publicDir:false,define:{'process.env.NODE_ENV':'"production"'},resolve:{dedupe:['react','react-dom']},build:{outDir:out,emptyOutDir:false,lib:{entry:path.join(__dirname,'mounted-harness.tsx'),formats:['iife'],name:'CharacterPreview',fileName:()=> 'app.js'}}});
