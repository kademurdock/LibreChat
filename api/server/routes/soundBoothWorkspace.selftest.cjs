...
(async()=>{
 ...
 try{
  ...
  await page.locator('.clips audio source').waitFor({state:'attached'});
  await page.locator('#btnRender').click();
  await page.locator('#renderConfirmation').waitFor();
  assert.equal(sent.at(-1).body.engine,'yue2');
  assert.equal(sent.at(-1).body.estimateOnly,true);
  ...
  assert.equal(sent.length,beforeCover+1,'cancelling a cover quote makes no generation request');
