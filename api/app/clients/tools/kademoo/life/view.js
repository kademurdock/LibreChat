...
const {
  MooRoom, MooItem, MooChar, MooDistrict, DIR_WORDS, clockLine, worldClock, kindOfSoul, coinOf,
} = require('./ctx');

async function describeRoom(ctx) {
  const { ch } = ctx;
  const room = await MooRoom.findOne({ roomId: ch.roomId }).lean();
  if (!room) return { roomId: null, name: 'Nowhere', desc: 'You are somewhere the world forgot to build. Say "home" or "go to gate" to be rescued.', exits: [], items: [], people: [], exitsDetail: [] };
  const [items, people, destRooms] = await Promise.all([
    ...
    MooRoom.find({ roomId: { $in: Object.values(room.exits || {}) } }).select('roomId name district props.home').lean(),
  ]);
  ...
  const loose = items.filter((i) => !(i.props && i.props.furniture) && !(i.props && i.props.vehicle));
  ...
  if (home) {
    ...
    if (furniture.length) desc += ' Furnished with ' + furniture.map((f) => f.name).join(', ') + '.';
    else desc += ' Bare floors — nothing in it yet.';
  }
  if (vehicles.length) desc += ' Parked here: ' + vehicles.map((v) => v.name + (v.props.ownerName ? ` (${v.props.ownerName}’s)` : '')).join(', ') + '.';
...
function personTag(ctx, p) {
  ...
  if (a.pose) tag = a.pose;
  ...
  else if (kind === 'citizen') { const d = reverie.npcDoingNow(p.userId); tag = d && d.doing ? d.doing : ''; }
    ...
    tag = idleMin > 30 ? 'here, but their mind is elsewhere' : idleMin > 8 ? 'quiet for a while' : '';
  }
  const line = tag ? (kind === 'stray' || a.pose ? `${p.name}, ${tag}` : `${p.name} (${tag})`) : p.name;
  return { id: p.userId, name: p.name, kind, tag, line, pronouns: a.pronouns || (kind === 'player' ? 'they' : null), cmds: personCmds(ctx, p, kind) };
}
...
function personCmds(ctx, p, kind) {
  ...
  if (kind === 'stray') return [f('Approach', 'approach'), f('Pet', 'pet'), f('Coax', 'coax'), f('Look at', 'look'), f('Carry', 'carry'), f('Adopt', 'adopt'), ...(p.attrs && p.attrs.owner === ctx.ch.userId ? [f('Bring along', 'bring'), f('Stay', 'stay')] : [])];
  if (kind === 'pet') return [f('Pet', 'pet'), f('Bring along', 'bring'), f('Stay', 'stay'), f('Look at', 'look')];
  if (kind === 'child') return [f('Talk to', 'talk to'), f('Play with', 'play with'), f('Feed', 'feed'), f('Read to', 'read to'), f('Teach', 'teach'), f('Tuck in', 'tuck in'), f('Bring along', 'bring'), f('Look at', 'look')];
...
async function hud(ctx) {
  ...
  const ward = room ? await MooDistrict.findOne({ districtId: room.district }).select('name').lean() : null;
  const wx = reverie.weatherNow();
  const c = worldClock();
  const top = skills.summary(life.skills || {}).filter((s) => s.level > 0).sort((a, b) => b.level - a.level).slice(0, 3);
  return {
    name: ch.name,
    ...
    weather: wx.kind,
    weatherLine: wx.line,
    ward: ward ? ward.name : (room ? room.district : ''),
...
async function actions(ctx) {
  ...
  for (const v of registry.all()) {
    ...
    try {
      ...
      for (const b of bs || []) out.push({ label: b.label, cmd: b.cmd, group: b.group || 'here', hint: b.hint || null });
    } catch (_) { /* a broken button never breaks a turn */ }
  }
...
async function decorate(ctx, result) {
  ...
  if (result.mode === 'play') {
    ...
    if (!result.people || !result.exits) {
      ...
      if (!result.room) result.here = { roomId: rv.roomId, name: rv.name, district: rv.district, home: rv.home };
    }
  } else {
