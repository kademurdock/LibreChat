...
class KadeGames extends Tool {
  ...
  /** Phase 4: shared with the Parlor routes — see kadegames/tableRunner.js. */
  async resolveSeatAgents(asks) {
    return resolveSeatAgents(this.userId, asks);
  }

  /** Phase 4 heart of it — shared with the Parlor routes (tableRunner.js):
   * personas (or botMove fallback) act until it's the human's turn. The chat
   * surface skips history collection; the conversation IS its log. */
  async playSeatTurns(doc, G) {
    return playSeatTurns({ userId: this.userId, doc, G, collectHistory: false });
  }
