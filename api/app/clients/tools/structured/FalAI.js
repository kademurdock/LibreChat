...
const falJsonSchema = falStudioSchema;
...
class FalAI extends Tool {
  constructor(fields = {}) {
    ...
    this.description_for_model =
       ...
       * agent instructions on purpose: every sound-capable agent inherits it
       * here, current and future, in one commit -- the same reasoning as the
       * autoTools fleet-roll. Every line below was paid for in failed takes
       * and graded by a blind listener, so none of it is theory.
       * Full receipts: PROJECT_STATUS PART 45.1. */
      "MAKING SOUND EFFECTS THAT ACTUALLY WORK (learned the hard way on this platform, ~90 files, every line measured not guessed): " +
      "(1) DESCRIBE THE SCENE, NOT THE WAVEFORM. A person, doing a thing, in a place. Prompts stuffed with texture verbs (RINGING, SNAPPED, zipping, crackling) produce texture soup that listeners identify as plastic bags and velcro. Seed Audio follows instructions faithfully, which means it follows BAD instructions faithfully too. " +
      "(2) ASK FOR A CROWD, NOT ONE OF THE THING. Want a footstep? Ask for someone walking a long way across that surface, then take one step out of the take. A single isolated instance comes out thin; a dense take gives many usable variants from one generation. " +
      "(3) INTERIORS NEED LAYERS. A one-shot 'busy diner' reads as a street or a train station every time. Generate the layers separately (room murmur alone, the grill alone), then mix them, quieter underneath. " +
      "(4) IT CANNOT DO INTIMATE HAND-FOLEY, and this is measured, not a hunch: coins in a palm and paper money handling failed every attempt, landing at 9-12kHz (thin, no body) where real coin sounds carry 1.5-3.4kHz. For anything small and close in the hands, a two-minute phone recording of the real thing beats anything generated. Say so plainly instead of burning attempts. " +
      "(5) A LOOP CONTAINS NOTHING THAT HAPPENS ONCE. Any background bed meant to repeat must have no single event in it -- one poured coffee becomes a poured coffee every eight seconds, forever. " +
      "(6) SEED AUDIO IS FOLEY AND SCENES, NOT MUSIC. A music request comes back as an object being dropped on a table. Real music is generate_song (Lyria 3 Pro or Minimax), a different action on this same tool. " +
      "(7) KEEP A MASTER LOSSLESS. If the clip is worth keeping or will be cut up later, set audio_quality:'high' -- same price, Seed bills by length not quality. Compress once, at the end, never twice.";
