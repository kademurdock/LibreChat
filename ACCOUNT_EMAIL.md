...
## Access approval, Part171

Approving creates a verified account through the existing registration path, then sends the actual sign-in identifier and newly generated temporary password with `accessApproved.handlebars`. It never sends a second verification message. An existing account receives login help with its password unchanged. Phone-only placeholder addresses receive no email; the administrator gets a manual sign-in message.

The request atomically claims one delivery attempt and records `sending`, `accepted`, or `unconfirmed`. Repeated approval cannot duplicate an accepted or uncertain send. Transport or mail-claim failure preserves the account and the manual message. Passwords are never stored on the request or in logs. Acceptance means the provider accepted delivery, not that the recipient read it. The exact registration-created user ID must match before a temporary password is sent.
