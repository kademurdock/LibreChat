...
## July 18 2026 — nightly restart + hang-up landing + skip-to-chat (Kade's asks, one commit)
- **Nightly memory-hygiene restart:** `api/server/services/kadeNightlyRestart.js` (+2 lines in `api/server/index.js`). Daily `process.exit(1)` at UTC hour 10 (5am CDT; deliberately NOT 9 — that's the Sunday consolidation sweep's hour) — Railway's restart policy revives the container in under a minute; the app's RAM footprint (85% of the Chat project's bill) resets daily instead of climbing all week. Guards: never within 2h of boot (deploys can't bounce themselves, no restart loops). Env: `KADE_NIGHTLY_RESTART=0` off, `KADE_NIGHTLY_RESTART_HOUR` retune.
- **Hang-up lands in the call's chat** (`ConversationMode.tsx`): after endCall, poll the convo list (1.8s + up to 3 retries) and navigate to the newest conversation touched since the call started — the streaming transcript is minted server-side moments after the socket closes. Fail-soft: no match within ~10s → stay put.
- **Skip-to-chat link** (`routes/Root.tsx`): very first focusable element on every page is now a visually-hidden-until-focused "Skip to the message box" link that focuses `#prompt-textarea`. Screen reader lands on it before the sidebar; one Enter and you're in the composer.
